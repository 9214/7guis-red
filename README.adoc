...
All seven challenges are complete and clock under {sloc} lines of code, barring script headers.
...
link:review.adoc[`review`]:: Case-study of the above-mentioned implementations; follows Eugen Kiss' https://github.com/eugenkiss/7guis/blob/master/thesis.pdf[thesis] methodology.
...
For a more hands-on and offline experience, clone this repository and tinker directly with sources. Assuming you're in project's root directory, the example above boils down to:
...
If interpreting is not enough and you wish to compile instead, download Red toolchain, then switch on encapping with release mode:
