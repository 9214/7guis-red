...
NOTE: TBD trichotomy of HTML/CSS/JS: structure, style, substance.
...
Word's binding, shared series buffers, actor blocks as bodies of function with tacitly assumed arguments `face` and `event` (+ no way to localize words inside the bodies), tacit assumption about the order of facet values in VID. Reactivity of facets. `pane` block and face's position in it, `parent`.
...
No separate namespace (modules?) for View/VID (names of widgets, words in actors &c), modification w/o `copy`, no type-checking for face's fields (repurpose https://gitter.im/red/red?at=5f24151036563560f2b4cb06[this]), Secure dialect as a "bondage & discipline" for this general class of problems + ownership system. Constants, mutability. Indexing in Draw with relative offsets (Circles).
