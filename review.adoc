...
To the best of author's knowledge,footnote:[Reviewing the official 7GUIs repository at commit https://github.com/eugenkiss/7guis/commit/dd339c88b7302e221340c8ee2335ce6124a88c54[`dd339c8`] and considering its pending pull requests.] only the implementations of 7GUIs tasks listed in the table below have completed all the seven challenges. Source lines of code (SLOC) metric of implementation files amd their dependencies was taken as displayed by Github code viewer.
...
| https://github.com/themoritz/7guis-reflex[Haskell, Reflex-DOM framework]
| 13
| 14
| 44
| 30
...
| https://github.com/foam-framework/foam/tree/master/js/foam/demos/sevenguis[JavaScript, FOAM framework]
| 22
| 33
| 103
| 99
...
[small]
. 87 SLOC of extra helper libraries.
. 161 SLOC of extra helper libraries.

---

- https://github.com/FLUXparticle/7guis

- https://github.com/eugenkiss/7guis-Clojure-Seesaw
- https://github.com/eugenkiss/7guis-Java7-Swing
- https://github.com/eugenkiss/7guis-Java8-JavaFX
- https://github.com/eugenkiss/7guis-React-TypeScript-MobX
- https://github.com/eugenkiss/7guis-Scala-ScalaFX

Red implementation is the smallest fully functional implementation in terms of the total SLOC count, even with the leg-up of ignoring library imports and not checking the implemented functionality (in the other implementations), and (in Red impl.) counting non-essential script headers, taking the helping code into consideration (e.g. undo/redo functionality in Circle Drawer, formula parser and evaluator in Cells) and a bit excessive code formatting (link to the official style guidelines, adhering to them) without intentional code-golfing.

Generally low level of diffuseness and boilerplate.

Metaprogramming (e.g. generating cells). Static and dynamic ways of creating reactive relations (cite the docs?).

Cells -- conditional expressions that guard the change of cell's state, FSM-like dialect. High degree of polymorphism, small set of actions working seamlessly across the board. Value literals can produce (express) the meaning of their own (e.g. RGB `tuple!`, hex `issue!`) -- syntax determines semantics. 
